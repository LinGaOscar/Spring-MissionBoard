...
- Apache POI 5.3.0（Excel 匯出）、springdoc-openapi 2.7.0（OpenAPI + Swagger UI）、spring-dotenv、Lombok
...
開啟 http://localhost:8080 登入。API 規格見 http://localhost:8080/swagger-ui/index.html （需先登入）。完整開發環境設定（環境變數、資料庫重置、測試帳號、常用指令）見 [docs/dev.md](docs/dev.md)。
...
| 文件 | 內容 |
|---|---|
| [docs/dev.md](docs/dev.md) | 本機開發環境建置、資料庫重置、測試帳號、API 文件路徑、常用指令 |
