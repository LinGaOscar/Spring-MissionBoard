...
**Architecture:** 純前端功能，全部使用既有、已測試過的後端 API（`GET .../tasks`、`GET .../task-categories`、`GET .../members`、`PUT .../tasks/{taskId}`），不動後端程式碼。新增 Vue 元件 `WbsView`，與現有 `KanbanView`/`AssignmentView` 平行、獨立 `mounted()` 時 fetch 自己的資料。實作前先把 `KanbanView` 內即將被 `WbsView` 共用的部分抽出來（比照現有 `toastMixin` 的抽法）：任務新增/編輯/刪除的邏輯抽成 `taskModalMixin`，modal 畫面本身（表單/按鈕的 HTML）因為 Vue mixin 不合併 template，另外抽成一個真正的子元件 `TaskModal`；tasks/categories/members 三份資料載入＋寫入序列化佇列抽成 `taskBoardMixin`。抽出後 `KanbanView` 行為不變（純重構）。根元件從雙分頁改三分頁（`v-if` 切換）。
...
## Task 1: 抽出 `taskModalMixin` 與 `TaskModal` 元件（純重構，看板行為不變）
...
- Consumes: 無新 API
- Produces:
  - 新 mixin `taskModalMixin`（data: `modal`；methods: `emptyForm()`、`openCreate()`、`openEdit(t)`、`saveTask()`、`deleteTask()`），供 `KanbanView` 使用，Task 2 會讓 `WbsView` 也引入同一份
  - 新元件 `TaskModal`（props: `modal: Object`、`members: Array`、`categories: Array`、`canWrite: Boolean`；emits: `save`、`delete`、`close`），全域註冊為 `task-modal`，供 `KanbanView` 使用，Task 2 會讓 `WbsView` 也用同一顆

現況 `KanbanView` 的 `data()`（第 55-79 行）把 `modal` 跟其他看板專屬的 state（`columns`/`dragging`/`categoryPanelOpen`/preset 相關/`draggingCategoryId` 等）混在一起，`methods` 裡的 `emptyForm`/`openCreate`/`openEdit`/`saveTask`/`deleteTask` 也跟看板專屬的拖曳/分類 CRUD 方法混在一起，`template` 裡還內嵌了一段約 37 行的 modal HTML。這次 `WbsView` 也要開同一顆任務編輯 modal：邏輯（data/methods）用 mixin 共用沒問題，但 Vue 的 mixin 機制不會合併 `template`——如果只抽 mixin、HTML 仍留在各自元件的 `template` 字串裡，`WbsView` 就得複製貼上同一段 37 行 markup，之後改一個表單欄位要兩邊同步改，是不必要的重複。因此這裡連 HTML 也一併抽成一個真正的 Vue 子元件 `TaskModal`，`KanbanView` 與 `WbsView` 的模板都只放一行 `<task-modal ... />`。
...
- [ ] **Step 2: 在 `taskModalMixin` 定義之後、`KanbanView` 定義之前，插入 `TaskModal` 元件**
...
- [ ] **Step 3: `KanbanView` 引入 mixin，移除已搬移的 data/methods**
...
- [ ] **Step 4: `KanbanView` 模板裡的 modal HTML 換成 `<task-modal>`**

現況（`template:` 字串裡的一段，原檔案第 461-497 行）：
```html
...
這一行放在原本那段 HTML 的位置（`</div>`｜`<div class="kanban">...</div>` 之後、`<div v-if="toastMessage" class="toast">` 之前），行為不變：`saveTask`/`deleteTask` 還是 `taskModalMixin` 提供的方法，`modal`/`members`/`categories`/`canWrite` 還是 `KanbanView` 自己的 state/prop，只是改成透過 `TaskModal` 元件的 props 傳進去、事件傳出來。

- [ ] **Step 5: 在檔尾 `app.component(...)` 註冊區塊加上 `task-modal`**
...
（全域註冊後，`KanbanView`／之後 Task 2 的 `WbsView` 的模板都能直接用 `<task-modal>`，不需要各自 `components: {...}` 局部註冊。）

- [ ] **Step 6: `mvn test` 確認全綠**
...
- [ ] **Step 7: 瀏覽器實測，確認看板 modal 行為沒有因重構而改變**
...
git commit -m "refactor: 抽出 taskModalMixin 與 TaskModal 元件，供看板與後續 WBS 分頁共用任務編輯 modal"
```
...
**Interfaces:**
- Consumes: `GET /api/projects/{id}/tasks`、`GET /api/projects/{id}/task-categories`、`GET /api/projects/{id}/members`、`PUT /api/projects/{id}/tasks/{taskId}`（皆為既有、已測試過的端點）；`taskModalMixin` 的 `modal`/`openEdit`/`saveTask`/`deleteTask` 與全域元件 `task-modal`（皆 Task 1 產出）
...
現況（Task 1 Step 5 已經在這裡加過 `task-modal` 註冊）：
```javascript
  const app = createApp({
    data() {
...
2. 點「WBS 檢視」，看到「未歸類 (1)」節點（`0%` 完成度，1 筆任務、0 筆已完成）與「SIT (2) 50%」→ 展開後看到子節點「程式開發 (2) 50%」，其下兩張任務列（「設計登入頁」標「已完成」、「實作看板拖曳」標「進行中」）
...
- [ ] Task 1 Step 7、Task 2 Step 6 的瀏覽器實測全數通過、Task 3 的文件更新完成即整體完成，無需額外收尾步驟。
